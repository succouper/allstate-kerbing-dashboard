...
function TenderCard({ tender, onFollowUp, pipelineData, setPipelineField }) {
  ...
  return (
    <div style={{ background: hasStatus ? (statusCfg?.bg || "#131210") : tender.id === 1 ? "rgba(201,169,110,0.04)" : "#131210", border:`1px solid ${hasStatus ? statusCfg?.color+"44" : tender.id === 1 ? "rgba(201,169,110,0.25)" : "#1E1C18"}`, borderLeft:`3px solid ${leftBorderColor}`, marginBottom:"3px" }}>
      <div onClick={() => setExpanded(!expanded)} style={{ display:"grid", gridTemplateColumns:"1fr 120px 80px 80px 90px 90px 110px 160px", gap:"8px", alignItems:"center", padding:"10px 14px", cursor:"pointer" }}>
        <div>
        ...
        <div onClick={e => e.stopPropagation()}>
          <select
            value={pd.status || ""}
            onChange={e => setPipelineField(tender.id, "status", e.target.value)}
            style={{ width:"100%", background: statusCfg ? statusCfg.bg : "#1A1812", border:`1px solid ${statusCfg ? statusCfg.color : "#2A2720"}`, color: statusCfg ? statusCfg.color : "#6B6560", fontFamily:"monospace", fontSize:"8px", letterSpacing:"0.5px", padding:"4px 6px", outline:"none", cursor:"pointer", borderRadius:"2px", fontWeight: statusCfg ? 700 : 400 }}
          >
      ...
      {expanded && (
        <div style={{ borderTop:"1px solid #1E1C18", background:"#0F0E0B" }}>
          ...
          <div onClick={e => e.stopPropagation()} style={{ borderTop:"1px solid #1E1C18", padding:"12px 14px", display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"12px" }}>
            <div>
              <div style={{ fontFamily:"monospace", fontSize:"8px", letterSpacing:"2px", textTransform:"uppercase", color:"#6B6560", marginBottom:"5px" }}>Owner / Responsible</div>
              <select value={pd.owner || ""} onChange={e => setPipelineField(tender.id, "owner", e.target.value)} style={{ width:"100%", background:"#1A1812", border:"1px solid #2A2720", color: pd.owner ? "#E8E0D0" : "#6B6560", fontFamily:"monospace", fontSize:"10px", padding:"5px 8px", outline:"none", borderRadius:"2px", cursor:"pointer" }}>
            ...
            <div>
              <div style={{ fontFamily:"monospace", fontSize:"8px", letterSpacing:"2px", textTransform:"uppercase", color:"#6B6560", marginBottom:"5px" }}>Internal Deadline</div>
              <input type="date" value={pd.deadline || ""} onChange={e => setPipelineField(tender.id, "deadline", e.target.value)} style={{ width:"100%", background:"#1A1812", border:"1px solid #2A2720", color: pd.deadline ? "#E8E0D0" : "#6B6560", fontFamily:"monospace", fontSize:"10px", padding:"5px 8px", outline:"none", borderRadius:"2px" }} />
            </div>
            <div>
              <div style={{ fontFamily:"monospace", fontSize:"8px", letterSpacing:"2px", textTransform:"uppercase", color:"#6B6560", marginBottom:"5px" }}>Internal Notes</div>
              <input type="text" value={pd.notes || ""} onChange={e => setPipelineField(tender.id, "notes", e.target.value)} placeholder="e.g. waiting on pricing..." style={{ width:"100%", background:"#1A1812", border:"1px solid #2A2720", color:"#E8E0D0", fontFamily:"monospace", fontSize:"10px", padding:"5px 8px", outline:"none", borderRadius:"2px" }} />
            </div>
          </div>
        </div>
      )}
...
function FollowUpModal({ tender, onClose, ejsConfig }) {
  ...
  const send = async () => {
    if (!ejsConfig?.publicKey || !ejsConfig?.serviceId || !ejsConfig?.templateId) {
      setStatus("error"); setErrMsg("Email not configured — go to ⚙ Email Setup tab first"); return;
    }
    setStatus("sending");
    try {
      emailjs.init({ publicKey: ejsConfig.publicKey });
      await emailjs.send(ejsConfig.serviceId, ejsConfig.templateId, {
        to_email: RECIPIENT, tender_name: tender.name, due_date: tender.closes,
        budget: tender.budget, region: tender.region, tender_site: tender.tenderSite,
        tender_type: tender.tenderType, kerb_scope: tender.kerbRelevance,
        urgency, notes: notes || "No additional notes.", from_name: "AllState Kerbing Dashboard",
      });
      setStatus("success");
      setTimeout(onClose, 2000);
    } catch(e) {
      setStatus("error"); setErrMsg(e.text || "Send failed — check EmailJS credentials in Setup tab");
    }
  };
    ...
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.82)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:"20px" }}>
      <div style={{ background:"#131210", border:"1px solid #2A2720", borderTop:"3px solid #C9A96E", width:"100%", maxWidth:"500px", padding:"28px 32px", position:"relative" }}>
        ...
        <div style={{ display:"flex", gap:"10px" }}>
          ...
          <button onClick={send} disabled={status === "sending" || status === "success"} style={{ flex:1, background:status === "success" ? "#2E7D5F" : "#C9A96E", border:"none", color:"#131210", fontFamily:"monospace", fontSize:"10px", letterSpacing:"1.5px", textTransform:"uppercase", padding:"12px", cursor:"pointer", fontWeight:700, opacity:status === "sending" ? 0.6 : 1, borderRadius:"2px" }}>
            {status === "sending" ? "Sending..." : status === "success" ? "✓ Sent!" : "Send Follow-Up Request"}
          </button>
        </div>
        {status === "error" && <div style={{ marginTop:"10px", fontFamily:"monospace", fontSize:"10px", color:"#E05C4A", textAlign:"center" }}>✕ {errMsg}</div>}
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const fetchFromSheets = async (url) => {
    ...
    try {
      ...
      const headers = parseCSV(lines[0]).map(h => h.replace(/^"|"$/g, "").trim());
      const parsed = lines.slice(1).map((line, idx) => {
        const vals = parseCSV(line); const obj = {};
        headers.forEach((h, i) => { obj[h] = (vals[i] || "").replace(/^"|"$/g, "").trim(); });
        return { id: Number(obj.id) || idx+1, name: obj.name||"", location: obj.location||"", builder: obj.builder||"", budget: obj.budget||"", budgetMin: parseFloat(obj.budgetMin)||0, budgetMax: parseFloat(obj.budgetMax)||0, distance: obj.distance||"", distanceKm: parseFloat(obj.distanceKm)||0, category: obj.category||"Other", closes: obj.closes||"", closesSort: parseInt(obj.closesSort)||99999999, region: obj.region||"", tenderSite: obj.tenderSite||"", tenderType: obj.tenderType||"", kerbRelevance: obj.kerbRelevance||"", urgent: String(obj.urgent).toUpperCase()==="TRUE", gc: String(obj.gc).toUpperCase()==="TRUE" };
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const parsed = raw.filter(r => r.id && !isNaN(Number(r.id))).map(r => ({ id:Number(r.id), name:String(r.name||""), location:String(r.location||""), builder:String(r.builder||""), budget:String(r.budget||""), budgetMin:Number(r.budgetMin)||0, budgetMax:Number(r.budgetMax)||0, distance:String(r.distance||""), distanceKm:Number(r.distanceKm)||0, category:String(r.category||"Other"), closes:String(r.closes||""), closesSort:Number(r.closesSort)||99999999, region:String(r.region||""), tenderSite:String(r.tenderSite||""), tenderType:String(r.tenderType||""), kerbRelevance:String(r.kerbRelevance||""), urgent:String(r.urgent).toUpperCase()==="TRUE", gc:String(r.gc).toUpperCase()==="TRUE" }));
        if (parsed.length === 0) throw new Error("No valid tender rows found");
  ...
  const filtered = useMemo(() => {
    ...
    return r;
  }, [tenders, search, filterRegion, filterCategory, filterSite, filterType, filterRelevance, filterUrgent, budgetRange, sortBy]);
  ...
  const loadPipelineData = async () => {
    const { data, error } = await supabase.from("pipeline_data").select("*");
    if (error) {
  ...
  const setPipelineField = async (tenderId, field, value) => {
    const updated = { ...pipelineData, [tenderId]: { ...(pipelineData[tenderId] || {}), [field]: value } };
    setPipelineData(updated);

    const pipelineRow = updated[tenderId];
    const { error } = await supabase
      .from("pipeline_data")
      .upsert({
        tender_id: tenderId,
        status: pipelineRow.status || "",
        owner: pipelineRow.owner || "",
        deadline: pipelineRow.deadline || null,
        notes: pipelineRow.notes || "",
  ...
  const activeTenders = tenders.filter(t => {
    ...
    return pd?.status && pd.status !== "Pass" && pd.status !== "";
  });
    ...
    <div style={{ minHeight:"100vh", background:"#0D0C0A", color:"#E8E0D0", fontFamily:"system-ui, sans-serif" }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:"4px", flexShrink:0 }}>
          {liveStatus === "live" && (
            <div style={{ display:"flex", alignItems:"center", gap:"5px", padding:"4px 10px", background:"rgba(46,125,95,0.08)", border:"1px solid rgba(46,125,95,0.3)", borderRadius:"2px", marginRight:"6px" }}>
              ...
              <button onClick={() => fetchFromSheets(sheetsUrl)} style={{ background:"none", border:"none", color:"#2E7D5F", fontSize:"10px", cursor:"pointer", padding:"0 2px" }}>↻</button>
            </div>
          )}
      ...
      {activeTab === "tenders" && (
        <div style={{ padding:"24px 32px" }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"1px", background:"#1E1C18", border:"1px solid #1E1C18", marginBottom:"20px" }}>
            {[{ n:tenders.length, label:"Total Tenders", sub:"kerb & channel keyword", color:"#C9A96E" },{ n:tenders.filter(t => t.kerbRelevance.startsWith("Direct")).length, label:"Direct Scope", sub:"highest ASK fit", color:"#2E7D5F" },{ n:tenders.filter(t => t.urgent).length, label:"Closing This Week", sub:"act now", color:"#E05C4A" },{ n:filtered.length, label:"Showing", sub:"after filters", color:"#8A8078" }].map(s => (
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 120px 80px 80px 90px 90px 110px 160px", gap:"8px", padding:"8px 14px", borderBottom:"1px solid #1E1C18", marginBottom:"4px" }}>
            {["Project / Region","Builder","Budget","Distance","K&C Scope","Closes","Status","Action"].map(h => (
          ...
          {filtered.length === 0
            ...
            : filtered.map(t => <TenderCard key={t.id} tender={t} onFollowUp={setModal} pipelineData={pipelineData} setPipelineField={setPipelineField} />)
          }
        ...
        <div style={{ padding:"28px 32px" }}>
          ...
          {activeTenders.length === 0 ? (
            <div style={{ background:"#131210", border:"1px solid #1E1C18", borderLeft:"3px solid #C9A96E", padding:"28px 32px", textAlign:"center" }}>
              ...
              <div style={{ fontFamily:"monospace", fontSize:"11px", color:"#6B6560", lineHeight:"1.7" }}>Go to <strong style={{ color:"#C9A96E" }}>Tender Intelligence</strong> → open any tender → set its Status to Reviewing, Bidding or Submitted.<br/>It will appear here automatically.</div>
            </div>
          ) : (
            <>
              {/* Kanban columns */}
                      ...
                      {group.map(t => {
                        ...
                        return (
                          <div key={t.id} style={{ background:"#131210", border:`1px solid ${cfg.color}44`, borderLeft:`3px solid ${cfg.color}`, padding:"16px 20px", display:"grid", gridTemplateColumns:"1fr 140px 120px 120px 160px", gap:"16px", alignItems:"center" }}>
                            ...
                            <div>
                              <div style={{ fontFamily:"monospace", fontSize:"8px", color:"#6B6560", letterSpacing:"1px", marginBottom:"4px" }}>CHANGE STATUS</div>
                              <select
                                value={pd.status || ""}
                                onChange={e => setPipelineField(t.id, "status", e.target.value)}
                                style={{ width:"100%", background:cfg.bg, border:`1px solid ${cfg.color}`, color:cfg.color, fontFamily:"monospace", fontSize:"9px", padding:"4px 8px", outline:"none", cursor:"pointer", borderRadius:"2px", fontWeight:700 }}
                              >
                                <option value="Reviewing">Reviewing</option>
                                <option value="Bidding">Bidding</option>
                                <option value="Submitted">Submitted</option>
                            ...
                            <div>
                              <div style={{ fontFamily:"monospace", fontSize:"8px", color:"#6B6560", letterSpacing:"1px", marginBottom:"4px" }}>OWNER</div>
                              <select
                                value={pd.owner || ""}
                                onChange={e => setPipelineField(t.id, "owner", e.target.value)}
                                style={{ width:"100%", background:"#1A1812", border:"1px solid #2A2720", color: pd.owner ? "#E8E0D0" : "#6B6560", fontFamily:"monospace", fontSize:"9px", padding:"4px 8px", outline:"none", cursor:"pointer", borderRadius:"2px" }}
                              >
                      ...
                      return (
                        <div key={t.id} style={{ background:"#131210", border:"1px solid #1E1C18", borderLeft:`3px solid ${cfg.color}`, padding:"10px 16px", display:"flex", justifyContent:"space-between", alignItems:"center", opacity:0.7 }}>
                          ...
                          <div style={{ display:"flex", gap:"10px", alignItems:"center" }}>
                            ...
                            <button onClick={() => setPipelineField(t.id, "status", "")} style={{ background:"none", border:"1px solid #2A2720", color:"#6B6560", fontFamily:"monospace", fontSize:"8px", padding:"2px 8px", cursor:"pointer", borderRadius:"2px" }}>Clear</button>
                          </div>
                        </div>
                      );
                    })}
          ...
          <div style={{ marginTop:"20px", padding:"12px 16px", background:"#131210", border:"1px solid #1E1C18", fontSize:"11px", color:"#6B6560", lineHeight:"1.7" }}>
            <strong style={{ color:"#E8E0D0" }}>How it works:</strong> Status and owner are saved in your browser. To share the pipeline with the team, export the pipeline data by copying it into the Excel sheet's Status column and re-importing. All statuses persist between sessions on the same device.
          </div>
        </div>
      )}
      ...
      {activeTab === "setup" && (
        <div style={{ padding:"28px 32px", maxWidth:"720px" }}>
          <PT title="Setup & Configuration" sub="EmailJS follow-up system · Live Google Sheets data feed · ~15 min total setup" />
          {ejsConfig?.publicKey && <div style={{ background:"#131210", border:"1px solid #1E1C18", borderLeft:"3px solid #2E7D5F", padding:"12px 16px", marginBottom:"20px", fontFamily:"monospace", fontSize:"11px", color:"#2E7D5F" }}>✓ Email is active — follow-up buttons are live</div>}
          <SL mt="0">EmailJS Configuration</SL>
          {[["1","Create free EmailJS account","Go to emailjs.com and sign up. Free plan = 200 emails/month."],["2","Add an Email Service","EmailJS Dashboard → Email Services → Add New Service. Connect Gmail or Outlook. Copy your Service ID."],["3","Create Email Template","Set To Email = tender@askerbing.com.au. Use variables: {{tender_name}}, {{urgency}}, {{notes}}, {{due_date}}, {{budget}}, {{region}}, {{tender_site}}. Copy Template ID."],["4","Get Public Key","EmailJS → Account → General → copy your Public Key."],["5","Paste credentials below","Enter all three IDs and click Save."]].map(([n,title,desc]) => (
          ...
          <Card style={{ marginTop:"16px", marginBottom:"32px" }}>
            <div style={{ fontFamily:"monospace", fontSize:"9px", letterSpacing:"2px", textTransform:"uppercase", color:"#6B6560", marginBottom:"16px" }}>Your EmailJS Credentials</div>
            {[["ejs-pk","Public Key","publicKey","e.g. user_aBcDeFgHi..."],["ejs-sid","Service ID","serviceId","e.g. service_xxxxxxx"],["ejs-tid","Template ID","templateId","e.g. template_xxxxxxx"]].map(([id,label,cfgKey,ph]) => (
              <div key={id} style={{ marginBottom:"12px" }}>
                <div style={{ fontFamily:"monospace", fontSize:"8px", letterSpacing:"2px", textTransform:"uppercase", color:"#6B6560", marginBottom:"5px" }}>{label}</div>
                <input id={id} type="text" defaultValue={ejsConfig?.[cfgKey] || ""} placeholder={ph} style={{ width:"100%", background:"#0D0C0A", border:"1px solid #2A2720", color:"#E8E0D0", fontFamily:"monospace", fontSize:"12px", padding:"9px 12px", outline:"none", borderRadius:"2px" }} />
              </div>
            ))}
            <button onClick={saveEjsConfig} style={{ background:"#C9A96E", border:"none", color:"#131210", fontFamily:"monospace", fontSize:"10px", letterSpacing:"1.5px", textTransform:"uppercase", padding:"11px 22px", cursor:"pointer", fontWeight:700, borderRadius:"2px" }}>Save & Activate</button>
          </Card>
      ...
      {modal && <FollowUpModal tender={modal} onClose={() => setModal(null)} ejsConfig={ejsConfig} />}
    </div>
    </>
  );
}
