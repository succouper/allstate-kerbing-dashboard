...
const isClosed = (t) => t.closesSort !== 99999999 && t.closesSort < TODAY_SORT;
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // Only parse rows after header+hint where first col is a real numeric ID
        const parsed = rawRows.slice(hdrRowIdx + 1)
          .filter(row => { const v = String(row[0]).trim(); return v && !isNaN(Number(v)) && Number(v) > 0; })
          .map((row, idx) => ({
