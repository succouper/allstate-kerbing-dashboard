...
function TenderCard({ tender, onFollowUp }) {
  const [expanded, setExpanded] = useState(false);
  const urgC = tender.urgent ? "#E05C4A" : "#8A8078";
  return (
    <div style={{ background:tender.id === 1 ? "rgba(201,169,110,0.04)" : "#131210", border:`1px solid ${tender.id === 1 ? "rgba(201,169,110,0.25)" : "#1E1C18"}`, borderLeft:`3px solid ${tender.kerbRelevance.startsWith("Direct") ? "#2E7D5F" : "#2A2720"}`, marginBottom:"3px" }}>
      <div onClick={() => setExpanded(!expanded)} style={{ display:"grid", gridTemplateColumns:"1fr 140px 90px 90px 100px 110px 145px", gap:"8px", alignItems:"center", padding:"10px 14px", cursor:"pointer" }}>
        <div>
      ...
      {expanded && (
        <div style={{ borderTop:"1px solid #1E1C18", padding:"12px 14px 14px", display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"12px", background:"#0F0E0B" }}>
    ...
    <div style={{ minHeight:"100vh", background:"#0D0C0A", color:"#E8E0D0", fontFamily:"system-ui, sans-serif" }}>
      ...
      {activeTab === "tenders" && (
        <div style={{ padding:"24px 32px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 140px 90px 90px 100px 110px 145px", gap:"8px", padding:"8px 14px", borderBottom:"1px solid #1E1C18", marginBottom:"4px" }}>
            {["Project / Region","Builder","Budget","Distance","K&C Scope","Closes","Action"].map(h => (
          ...
          {filtered.length === 0
            ...
            : filtered.map(t => <TenderCard key={t.id} tender={t} onFollowUp={setModal} />)
          }
